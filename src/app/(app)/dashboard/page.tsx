...
export default function DashboardPage() {
  ...
  const [editingContext, setEditingContext] = useState<{ principleId: string; field: 'whenTested' | 'howToHold' } | null>(null);
                        ...
                        >
                          <div className="px-4 pb-4 pt-2 border-t border-slate-100">
                            {hasContext ? (
                              <div className="space-y-3">
                                {/* When Tested Section */}
                                <div className="flex gap-3 group/context">
                                  ...
                                  <div className="flex-1">
                                    <div className="flex items-center justify-between mb-1">
                                      <p className="text-xs font-medium text-amber-700 uppercase tracking-wide">When it&apos;s tested</p>
                                      {editingContext?.principleId !== principle.id || editingContext?.field !== 'whenTested' ? (
                                        <button
                                          onClick={() => {
                                            setEditingContext({ principleId: principle.id, field: 'whenTested' });
                                            setContextInputValue(principle.whenTested || '');
                                          }}
                                          className="p-1 text-slate-400 hover:text-amber-600 rounded opacity-0 group-hover/context:opacity-100 transition-opacity"
                                        >
                                          <Pencil className="w-3 h-3" />
                                        </button>
                                      ) : null}
                                    ...
                                    {editingContext?.principleId === principle.id && editingContext?.field === 'whenTested' ? (
                                      <div className="space-y-2">
                                        <textarea
                                          ...
                                          className="w-full text-sm border border-amber-200 rounded-lg px-3 py-2 focus:outline-none focus:border-amber-400 resize-none"
                                          rows={3}
                                          autoFocus
                                          placeholder="Describe situations where this principle gets challenged..."
                                        />
                                        <div className="flex gap-2">
                                          <button
                                            onClick={() => {
                                              updatePrincipleContext(principle.id, { whenTested: contextInputValue });
                                      ...
                                      <p className="text-sm text-muted-foreground">
                                        {principle.whenTested || <span className="italic text-slate-400">Click to add...</span>}
                                      </p>
                                    )}
                                  </div>
                                </div>
                                ...
                                <div className="flex gap-3 group/context">
                                  ...
                                  <div className="flex-1">
                                    <div className="flex items-center justify-between mb-1">
                                      <p className="text-xs font-medium text-emerald-700 uppercase tracking-wide">How to hold it</p>
                                      {editingContext?.principleId !== principle.id || editingContext?.field !== 'howToHold' ? (
                                        <button
                                          onClick={() => {
                                            setEditingContext({ principleId: principle.id, field: 'howToHold' });
                                            setContextInputValue(principle.howToHold || '');
                                          }}
                                          className="p-1 text-slate-400 hover:text-emerald-600 rounded opacity-0 group-hover/context:opacity-100 transition-opacity"
                                        >
                                          <Pencil className="w-3 h-3" />
                                        </button>
                                      ) : null}
                                    ...
                                    {editingContext?.principleId === principle.id && editingContext?.field === 'howToHold' ? (
                                      <div className="space-y-2">
                                        <textarea
                                          value={contextInputValue}
                                          onChange={(e) => setContextInputValue(e.target.value)}
                                          className="w-full text-sm border border-emerald-200 rounded-lg px-3 py-2 focus:outline-none focus:border-emerald-400 resize-none"
                                          rows={3}
                                          autoFocus
                                          placeholder="Describe how to live up to this principle in practice..."
                                        />
                                        <div className="flex gap-2">
                                          <button
                                            onClick={() => {
                                              updatePrincipleContext(principle.id, { howToHold: contextInputValue });
                                              setEditingContext(null);
                                            }}
                                      ...
                                      <p className="text-sm text-muted-foreground">
                                        {principle.howToHold || <span className="italic text-slate-400">Click to add...</span>}
                                      </p>
                                    )}
                                  </div>
                                </div>
                              ...
                              <div className="space-y-4">
                                  ...
                                  <div className="flex-1">
                                    ...
                                    {editingContext?.principleId === principle.id && editingContext?.field === 'whenTested' ? (
                                      ...
                                      <button
                                        ...
                                        className="text-sm text-slate-400 hover:text-amber-600 italic"
                                      >
                                        + Add when this principle is tested
                                      </button>
                                    )}
                                  </div>
                                </div>

                                {/* How to Hold - Empty State */}
                                <div className="flex gap-3 group/context">
                                  <div className="w-6 h-6 rounded-lg bg-emerald-100 flex items-center justify-center flex-shrink-0 mt-0.5">
                                    <Crosshair className="w-3.5 h-3.5 text-emerald-600" />
                                  </div>
                                  <div className="flex-1">
                                    ...
                                    {editingContext?.principleId === principle.id && editingContext?.field === 'howToHold' ? (
                                      ...
                                      <button
                                        ...
                                        className="text-sm text-slate-400 hover:text-emerald-600 italic"
                                      >
                                        + Add how to hold this principle
                                      </button>
                                    )}
                                  </div>
                                </div>
                                ...
                                <div className="pt-2 border-t border-slate-100">
                                  ...
                                  >
                                    <Sparkles className="w-3 h-3" />
                                    Or let Willson help you think through this
                                  </button>
                                </div>
                              </div>
                            )}
